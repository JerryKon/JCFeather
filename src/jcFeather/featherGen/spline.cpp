...
//
{
  ...
  return mbasis;
	...
	void basis_matrix_overhauser_nonuni_jc(double alpha, double beta,double* mbasis)
	{
	  ...
	  mbasis[1+2*4] = ( 2.0 * ( 1.0 - alpha ) - beta * ( 1.0 - 2.0 * alpha ) ) / ( 1.0 - beta );
	...
	//
	  ...
	  else
	  {
		...
		if ( !ypp )
		{
		  cerr << "\n";
		  cerr << "SPLINE_CUBIC_SET - Fatal error!\n";
		  cerr << "  The linear system could not be solved.\n";
		  delete [] a;
		  delete [] b;
		  return NULL;
		}
...
//
  ...
  return int_val;
	...
	void spline_linear_intset ( int n, double int_x[], double int_v[], double data_x[], double data_y[] )
