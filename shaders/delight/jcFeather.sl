...
surface jcFeather(float jcFeaAmb = .5 ,
						 jcFeadiff = 0.6, 
						 jcFeaspec = .8, 
						 jcFeagloss = .07,
						 jcFeaselfshad=1,
				   ...
				   color jcFeaspecColor=1,
				   		 ...
				   		 jcFeaRachisTipC=0.4;
				   uniform float jcFeaBarU=0,jcFeaBarLenPer=1;
				   string jcFeaColorTex="None";)
 { 
 ...
 float nondiff=0, nonspec=0;
  ...
  /*-----------------------------------get the color and alpha of barbule texture*/   
 color texColor= color(1,1,1); 
 float aplha=1;
 
 //-----------if jcFeaBarU<0,this curve is rachis, if [0,1] it is left barbule,if [1,2] it is right barbule
 uniform float jcFeaSide=0,feaBarU=jcFeaBarU;
 if( feaBarU<0 ) 
 	jcFeaSide=-1;
 else if(feaBarU>=0&&feaBarU<=1)  
 	jcFeaSide=0;
 else 
 {
	 jcFeaSide=1;
 ...
 if( jcFeaColorTex!="" && jcFeaColorTex!="None")
 {  
	 float feaBarV=0;
	 ...
	 if(jcFeaSide!=-1)	
	 {
			 //if current curve is barbule curve,then get pixel of the texture.
 ...
 /*---------------------------------get the color and alpha of barbule texture*/
 float fadeSmooth=v-jcFeaFadeStart;
 float smsFade = smoothstep(0,1,fadeSmooth/(1-jcFeaFadeStart)); 
 Oi=aplha*mix(jcFeaBaseOpacity,jcFeaFadeOpacity,smsFade);
 

 
 float shadow_pass = 0;
 option( "user:ShadowMapRendering", shadow_pass );
 ...
 else
 {
	 ...
	 illuminance (P) 
	   {
		    ...
		    Cl2 = Cl*jcFeaselfshad+(1-jcFeaselfshad); /* limits the gamut of shadowing */
	 ...
	 if(jcFeaSide!=-1) 	 
	 {
	 	 mixed *= mix( jcFeaRootC*jcFeaMainC ,jcFeaTipC*jcFeaMainC ,v );
	 	 mixed *= texColor;
	 ...
	 Ci = mixed*Oi *Cdiff + Cspec * Oi*jcFeaspecColor;
 }
}